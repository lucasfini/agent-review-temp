...
import { calculateServiceCost } from '@/lib/billing/cost-map';
...
describe('AssemblyAI Duration Cost Calculations', () => {
  test('should calculate cost for 1 minute of audio', () => {
    ...
    const providerRate = 0.21 / 3600; // $0.21 per hour (Universal-3 Pro)

    const rawCost = durationSeconds * providerRate;
    const billedCost = calculateServiceCost('assemblyai_transcription', durationSeconds).billedCost;

    expect(rawCost).toBeCloseTo(0.0035, 6);
    expect(billedCost).toBeCloseTo(0.0065, 6);
  });

  test('should calculate cost for 1 hour of audio', () => {
    const durationSeconds = 3600;
    const providerRate = 0.21 / 3600; // $0.21/hour (Universal-3 Pro)

    const rawCost = durationSeconds * providerRate;
    const billedCost = calculateServiceCost('assemblyai_transcription', durationSeconds).billedCost;

    expect(rawCost).toBeCloseTo(0.21, 4);
    expect(billedCost).toBeCloseTo(0.39, 4);
  });
  ...
  test('should calculate cost for 30 minute podcast', () => {
    ...
    expect(rawCost).toBeCloseTo(0.105, 4);
    expect(billedCost).toBeCloseTo(0.195, 5);
  });
});
...
describe('Batch Usage Aggregation', () => {
  test('should calculate total cost for multiple services', () => {
    const usageEvents = [
      { serviceKey: 'assemblyai_transcription', billedCost: 0.39 },
    ...
    expect(totalBilledCost).toBeCloseTo(0.496, 4);
  });

  test('should group usage by provider', () => {
    const usageEvents = [
      { provider: 'assemblyai', billedCost: 0.39, units: 3600 },
...
describe('Pre-flight Estimate Accuracy', () => {
  test('should estimate within 20% of actual cost', () => {
    ...
    const estimatedCost = calculateServiceCost('assemblyai_transcription', estimatedDurationSeconds).billedCost;

    // Actual duration (slightly different)
    const actualDurationSeconds = 280; // 4 minutes 40 seconds
    const actualCost = calculateServiceCost('assemblyai_transcription', actualDurationSeconds).billedCost;
