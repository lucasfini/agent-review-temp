...
import {
  ...
  createAgencyProductionTask,
  listAgencyProductionTasks,
...
function errorResponse(error: unknown, fallback: string) {
  ...
  console.error('[AGENCY_PRODUCTION_TASKS] Unexpected error:', error);
...
async function requireProductionTaskAccess(request: NextRequest, params: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { organization, membership } = await requireProductionTaskAccess(request, {
      requestedOrganizationId: requestedOrganizationIdFrom(request),
      clientId,
    });
    const tasks = await listAgencyProductionTasks(supabaseAdmin, organization.id, {
      clientId,
    ...
    return errorResponse(error, 'Failed to load production tasks');
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const clientId = productionTaskClientIdFrom(body);
    const { user, organization, membership } = await requireProductionTaskAccess(request, {
    ...
    const task = await createAgencyProductionTask(supabaseAdmin, organization.id, user.id, body);
    ...
    return errorResponse(error, 'Failed to create production task');
  }
}
