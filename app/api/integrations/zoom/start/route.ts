...
import { INTEGRATIONS_COMING_SOON_MESSAGE, ZOOM_INTEGRATION_ENABLED } from '@/lib/integrations/availability';
...
export async function GET(request: NextRequest) {
  ...
  response.cookies.set('zoom_oauth_state', state, { httpOnly: true, sameSite: 'lax', maxAge: 600, path: '/' });
  response.cookies.set('zoom_oauth_user', user.id, { httpOnly: true, sameSite: 'lax', maxAge: 600, path: '/' });
  return response;
}
