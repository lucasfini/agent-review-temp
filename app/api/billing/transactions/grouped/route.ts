...
interface GroupedTransactionChild {
  reason: string;
  amount: number;
  createdAt: string;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reservationIds = Array.from(
      new Set(
        transactions
          .map((tx: any) => tx.reservation_id)
          .filter(Boolean)
      )
    );

    const usageEventIds = Array.from(
      new Set(
        transactions
          .map((tx: any) => tx.usage_event_id)
          .filter(Boolean)
      )
    );
    ...
    if (reservationIds.length > 0) {
      ...
      for (const reservation of reservations || []) {
        ...
        if (reservation.project_id) {
          projectIds.add(reservation.project_id);
        }
      }
    }
    ...
    for (const [reservationId, txs] of reservationTransactions.entries()) {
      ...
      const projectId =
        ...
        latestTx.metadata?.projectId ||
        usageEventProjectMap[latestTx.usage_event_id];
      ...
      const releasedAmount = Number(
        releaseTxs.reduce((sum, tx) => sum + Math.max(0, Number(tx.amount || 0)), 0).toFixed(4)
      );

      const children: GroupedTransactionChild[] = [];
      if (holdAmount > 0 && reserveTx) {
        children.push({
          reason: reserveTx.reason || 'Estimated hold',
          amount: -holdAmount,
          createdAt: reserveTx.created_at,
          kind: 'hold',
        });
      }
      if (finalCharge > 0) {
        children.push({
          reason: 'Final charge based on actual usage',
          amount: -finalCharge,
          createdAt: settleTx?.created_at || latestTx.created_at,
          kind: 'charge',
        });
      }
      if (releasedAmount > 0) {
        children.push({
          reason: 'Released back to balance',
          amount: releasedAmount,
          createdAt: releaseTxs[0]?.created_at || latestTx.created_at,
          kind: 'release',
        });
      }
