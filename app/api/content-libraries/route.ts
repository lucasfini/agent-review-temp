...
import {
  ...
  listContentLibrariesForOrganizations,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const contentLibraries = (await listContentLibrariesForOrganizations(supabaseAdmin, context.organizationIds))
      .map((library) => decorateContentLibraryScope(library, {
    ...
    return errorResponse(error, 'Failed to load libraries');
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!can({
      ...
      organizationType: context.organization.type,
      role: context.membership.role,
    }, 'collection.create', { organizationId: context.activeOrganizationId })) {
      return NextResponse.json({ error: 'You do not have permission to create collections' }, { status: 403 });
    }

    const contentLibrary = decorateContentLibraryScope(
      await createContentLibrary(supabaseAdmin, context.privateOrganizationId, context.user.id, body),
      {
        activeOrganizationId: context.activeOrganizationId,
        privateOrganizationId: context.privateOrganizationId,
        userId: context.user.id,
    ...
    return errorResponse(error, 'Failed to create library');
  }
}
